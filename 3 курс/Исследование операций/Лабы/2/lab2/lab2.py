...
def BaB_method(b_f, b_lts_a, b_limits_b, b_limits_type, v_num, l_num, f_type, b_fin_res, ti = 0, tj = 0, m = "", tbx = 0):
    f = copy.deepcopy(b_f)
    ...
    for ir, r in enumerate(res):
        if not float(round(r, pres)).is_integer():
            ...
            for func in fc_funcs:
                ...
                fin_res = BaB_method(f, lna, lnb, lnt, v_num, l_num+1, f_type, fin_res, ti+1, tj+it, tm, tbx+1)
                it += 1
...
if __name__ == '__main__':
    ...
    b_res = BaB_method(f_in, limits_a_in, limits_b_in, limits_type_in, v_num_in, l_num_in, f_type_in, None, 0)
